...
import { X, CheckCircle2, Upload, ArrowRight, AlertCircle, Plus, Shield } from 'lucide-react';
import { useApp } from '../../context/AppContext';

export const FileClaimModal: React.FC = () => {
  const { isFileClaimOpen, setIsFileClaimOpen, policies, addClaim, user, setIsAddPolicyOpen } = useApp();
  ...
  const policyTitleToUse = selectedPolicy?.title || customPolicyName.trim() || 'Insurance Policy';
  const insurerName = selectedPolicy?.insurer || 'Claims Processing Desk';

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrorMessage('');
    const amount = parseFloat(claimAmount);
    if (!hospitalName.trim() || isNaN(amount) || amount <= 0) {
      setErrorMessage('Please provide a valid hospital name and claim amount.');
      return;
    }

    if (!selectedPolicy && !customPolicyName.trim()) {
      setErrorMessage('Please select or specify your insurance policy.');
      return;
    }

    setIsSubmitting(true);
    const result = await addClaim({
      policyId: selectedPolicy?.id || `pol-${Date.now()}`,
      policyTitle: policyTitleToUse,
      patientName: patientName.trim() || user.name,
      hospitalName: hospitalName.trim(),
      claimAmount: amount,
      diagnosisOrReason: diagnosisOrReason.trim() || 'Medical Treatment',
      hospitalizationDate,
    ...
    if (result.success) {
      ...
      setErrorMessage(result.error || 'Failed to file claim with insurer desk.');
    }
  };
          ...
          {step === 1 && (
            <div className="space-y-4">
                ...
                {policies.length > 0 ? (
                  ...
                  <div className="space-y-2">
                    <p className="text-xs text-slate-500">
                      No linked policy found. Enter your policy name below or link a plan.
                    </p>
                    <input
                      type="text"
                      placeholder="e.g. Health Floater Policy Name & Number"
                    ...
                    >
                      ...
                      <span>Or link an insurance policy first</span>
                    </button>
                  </div>
                )}
              </div>
              ...
              <button
                ...
                onClick={() => {
                  if (!selectedPolicy && !customPolicyName.trim()) {
                    setErrorMessage('Please select or specify a policy.');
                    return;
                  }
                  setStep(2);
                }}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Diagnosis or treatment summary"
              ...
              <div>
                ...
                >
                  ...
                  <span>
                    {uploadedFilesCount} Document(s) Attached (Tap to add)
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  className="w-2/3 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl text-xs font-semibold cursor-pointer transition-all shadow-xs"
                >
            ...
            <div className="text-center py-4 space-y-3">
              ...
              <div className="bg-slate-50 dark:bg-slate-900/80 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs text-left space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{policyTitleToUse}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-blue-600 dark:text-blue-400">Under Review</span>
                </div>
              </div>
