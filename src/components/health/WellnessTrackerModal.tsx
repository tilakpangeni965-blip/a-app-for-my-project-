...
export const WellnessTrackerModal: React.FC = () => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const existingIndex = (wellness.weeklyTrend || []).findIndex((d) => d.day === todayDay);
    let updatedTrend: HealthTrendDay[];

    const todayEntry: HealthTrendDay = {
      day: todayDay,
      date: todayDate,
      steps,
      waterMl,
    ...
    if (existingIndex >= 0) {
      updatedTrend = [...(wellness.weeklyTrend || [])];
      updatedTrend[existingIndex] = todayEntry;
    } else {
      updatedTrend = [...(wellness.weeklyTrend || []), todayEntry].slice(-7);
    }
    ...
    switch (selectedMetric) {
      case 'steps':
        return {
          ...
          avgVal: trendData.length
            ? Math.round(trendData.reduce((acc, d) => acc + d.steps, 0) / trendData.length).toLocaleString()
            : '--',
          icon: Footprints,
      ...
      case 'water':
        return {
          ...
          avgVal: trendData.length
            ? `${Math.round(trendData.reduce((acc, d) => acc + d.waterMl, 0) / trendData.length)} ml`
            : '--',
          icon: Droplets,
      ...
      case 'sleep':
        return {
          ...
          avgVal: trendData.length
            ? `${(trendData.reduce((acc, d) => acc + d.sleepHours, 0) / trendData.length).toFixed(1)} hrs`
            : '--',
          icon: Moon,
      ...
      case 'heartRate':
        return {
          ...
          avgVal: trendData.length
            ? `${Math.round(trendData.reduce((acc, d) => acc + d.heartRateBpm, 0) / trendData.length)} bpm`
            : '--',
          icon: Heart,
        ...
        <div className="p-5 overflow-y-auto space-y-4 text-left">
          {activeView === 'chart' ? (
            <div className="space-y-4">
              ...
              {/* Recharts 7-Day Interactive Area Chart or Empty State */}
              {trendData.length === 0 ? (
                <div className="py-10 px-4 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-slate-50/50 dark:bg-slate-900/30">
                  <Activity className="w-8 h-8 text-slate-300 dark:text-slate-600 mx-auto mb-2" />
                  <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                    No weekly trend data logged yet
                  </p>
                  <p className="text-[11px] text-slate-500 mt-1 max-w-xs mx-auto">
                    Record your daily readings to view your 7-day health trend chart and rolling averages.
                  </p>
                  <button
                    type="button"
                    onClick={() => setActiveView('log')}
                    className="mt-3 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold cursor-pointer transition-colors shadow-xs"
                  >
                    Log Today&apos;s Reading
                  </button>
                </div>
