...
export const HerbalGuideModal: React.FC = () => {
  ...
  const [aiMessages, setAiMessages] = useState<Array<{ role: 'user' | 'assistant'; text: string; remedyRef?: string }>>([
    {
      role: 'assistant',
      text: `Hello! I am your LifeShield Herbal Preparation & Safety Assistant.

I can guide you on traditional preparation techniques (decoctions, infusions, teas, and golden milk), culinary ingredient proportions, and critical drug-herb contraindications.

How can I help you safely prepare a herbal remedy today?`
    }
  ]);
    ...
    try {
      const res = await fetch('/api/herbal-assistant', {
        ...
        body: JSON.stringify({
          ...
          userContext: {
            app: 'LifeShield',
            activeHerb: expandedId
          }
        })
      });

      const data = await res.json();
      const replyText = data.reply || data.fallback || 'Here is the general home preparation method for your request. Please exercise caution.';
      ...
      setAiMessages((prev) => [
        ...
        {
          role: 'assistant',
          text: `• Recommended Preparation: Boil 4-5 fresh leaves or 1/2 tsp culinary powder in 250ml water for 6-8 minutes. Strain and consume warm.\n• Safety Precaution: If you take prescribed pharmaceuticals (especially blood thinners or diabetes medication), herbal supplements can alter efficacy. Consult a licensed clinician before regular use.`
        }
      ]);
    } finally {
      setAiLoading(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-black/60 backdrop-blur-xs">
      <div className="bg-white dark:bg-[#0f172a] w-full max-w-lg rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        {/* Tab 2: AI Preparation Assistant (Gemini powered) */}
        {activeTab === 'ai_assistant' && (
          <div className="flex-1 overflow-y-auto flex flex-col justify-between p-4 space-y-3 text-left">
            {/* Quick Prompt Starters */}
            <div className="space-y-1.5">
              <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block">
                Suggested Herbal Inquiries:
              </span>
              <div className="flex flex-wrap gap-1.5">
                {[
                  'How to prepare Tulsi ginger tea for cold?',
                  'Is Turmeric golden milk safe with blood thinners?',
                  'How to make fresh ginger lemon tea?',
                  'Ashwagandha bedtime milk recipe & safety'
                ].map((prompt, i) => (
                  <button
                    key={i}
                    type="button"
                    onClick={() => handleAskAi(prompt)}
                    className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-950/50 text-[11px] text-slate-700 dark:text-slate-300 hover:text-blue-600 transition-colors cursor-pointer border border-slate-200/60 dark:border-slate-700"
                  >
                    {prompt}
                  </button>
                ))}
            ...
            {/* Conversation Log */}
            <div className="flex-1 overflow-y-auto space-y-3 min-h-[220px] max-h-[380px] p-2 bg-slate-50/50 dark:bg-slate-900/40 rounded-2xl border border-slate-200/80 dark:border-slate-800">
              ...
              {aiLoading && (
                <div className="flex items-center gap-2 p-2 text-xs text-blue-600 dark:text-blue-400">
                  ...
                  <span>Checking safe preparation ratios and contraindications...</span>
                </div>
              )}
            </div>
            ...
            >
              <input
                type="text"
                placeholder="Ask about recipe, boiling time, dosage, or drug clashes..."
                value={aiQuery}
